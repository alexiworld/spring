...
import com.example.springjdbcmybatis3.vehicle.dao.repository.VehicleEntityXMLRepository;
...
public class VehicleServiceRepoImpl implements VehicleService {
    ...
    public Vehicle fetchVehicleByVin(String vinNumber) {
        Optional<Vehicle> result = vehicleEntityXMLRepository.findByVinNumber(vinNumber).map(ModelUtils::map);
        if(result.isPresent()){
            return result.get();
        }
    ...
    public List<Vehicle> fetchVehiclesByAgent(String agent) {
        List<VehicleEntity> result = vehicleEntityXMLRepository.findAllByCreatedBy(agent);
        return result.stream().map(ModelUtils::map).toList();
    }
